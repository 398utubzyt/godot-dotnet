...
namespace Godot
{
    ...
    public struct Transform3D : IEquatable<Transform3D>
    {
        ...
        public readonly Transform3D InterpolateWith(Transform3D transform, Real weight)
        {
        ...
        public readonly Transform3D Rotated(Vector3 axis, Real angle)
        {
        ...
        /// <param name="angle">The angle to rotate, in radians.</param>
        /// <returns>The rotated transformation matrix.</returns>
        public readonly Transform3D RotatedLocal(Vector3 axis, Real angle)
        {
        ...
        public Transform3D(Real xx, Real yx, Real zx, Real xy, Real yy, Real zy, Real xz, Real yz, Real zz, Real ox, Real oy, Real oz)
        {
        ...
        public static Aabb operator *(Transform3D transform, Aabb aabb)
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                for (int j = 0; j < 3; j++)
                {
                    Real e = transform.Basis[i][j] * min[j];
                    Real f = transform.Basis[i][j] * max[j];
                    if (e < f)
                    {
                        tmin[i] += e;
                        tmax[i] += f;
