...
namespace Godot
{
    ...
    public struct Vector2 : IEquatable<Vector2>
    {
        ...
        /// </summary>
        public readonly void Deconstruct(out Real x, out Real y)
        {
            x = X;
            y = Y;
        }
        ...
        internal void Normalize()
        {
            ...
            {
                Real length = Mathf.Sqrt(lengthsq);
                X /= length;
                Y /= length;
            }
        }
        ...
        /// </summary>
        /// <returns>A vector with <see cref="Mathf.Abs(Real)"/> called on each component.</returns>
        ...
        ///
        /// Equivalent to the result of <see cref="Mathf.Atan2(Real, Real)"/> when
        ...
        public readonly Real AngleTo(Vector2 to)
        {
        ...
        /// <param name="to">The other vector to compare this vector to.</param>
        /// <returns>The angle between the two vectors, in radians.</returns>
        public readonly Real AngleToPoint(Vector2 to)
        {
        ...
        /// <see cref="Mathf.Clamp(Real, Real, Real)"/>.
        /// </summary>
        ...
        public readonly Real Cross(Vector2 with)
        {
        ...
        public readonly Vector2 CubicInterpolate(Vector2 b, Vector2 preA, Vector2 postB, Real weight)
        {
            return new Vector2
            (
        ...
        /// <returns>The interpolated vector.</returns>
        public readonly Vector2 CubicInterpolateInTime(Vector2 b, Vector2 preA, Vector2 postB, Real weight, Real t, Real preAT, Real postBT)
        {
            return new Vector2
            (
        ...
        /// <returns>The interpolated vector.</returns>
        public readonly Vector2 BezierInterpolate(Vector2 control1, Vector2 control2, Vector2 end, Real t)
        {
            return new Vector2
            (
        ...
        public readonly Vector2 BezierDerivative(Vector2 control1, Vector2 control2, Vector2 end, Real t)
        {
            return new Vector2(
        ...
        public readonly Real DistanceSquaredTo(Vector2 to)
        {
        ...
        public readonly Real DistanceTo(Vector2 to)
        {
        ...
        public readonly Real Dot(Vector2 with)
        {
        ...
        public readonly Vector2 Lerp(Vector2 to, Real weight)
        {
            return new Vector2
            (
        ...
        public readonly Vector2 LimitLength(Real length = 1.0f)
        {
            Vector2 v = this;
            Real l = Length();
        ...
        public readonly Vector2 MoveToward(Vector2 to, Real delta)
        {
            Vector2 v = this;
            Vector2 vd = to - v;
            Real len = vd.Length();
        ...
        /// <summary>
        /// Returns a vector composed of the <see cref="Mathf.PosMod(Real, Real)"/> of this vector's components
        /// and <paramref name="mod"/>.
        /// </summary>
        /// <param name="mod">A value representing the divisor of the operation.</param>
        /// <returns>
        /// A vector with each component <see cref="Mathf.PosMod(Real, Real)"/> by <paramref name="mod"/>.
        /// </returns>
        public readonly Vector2 PosMod(Real mod)
        {
            Vector2 v;
        ...
        /// <returns>
        /// A vector with each component <see cref="Mathf.PosMod(Real, Real)"/> by <paramref name="modv"/>'s components.
        /// </returns>
        ...
        public readonly Vector2 Rotated(Real angle)
        {
            (Real sin, Real cos) = Mathf.SinCos(angle);
            return new Vector2
            (
                X * cos - Y * sin,
                X * sin + Y * cos
        ...
        /// by calling <see cref="Mathf.Sign(Real)"/> on each component.
        /// </summary>
        ...
        /// having zero length, this method behaves like <see cref="Lerp(Vector2, Real)"/>.
        /// </summary>
        /// <param name="to">The destination vector for interpolation.</param>
        /// <param name="weight">A value on the range of 0.0 to 1.0, representing the amount of interpolation.</param>
        /// <returns>The resulting vector of the interpolation.</returns>
        public readonly Vector2 Slerp(Vector2 to, Real weight)
        {
            Real startLengthSquared = LengthSquared();
            Real endLengthSquared = to.LengthSquared();
            if (startLengthSquared == 0.0 || endLengthSquared == 0.0)
            {
                // Zero length vectors have no angle, so the best we can do is either lerp or throw an error.
                return Lerp(to, weight);
            }
            Real startLength = Mathf.Sqrt(startLengthSquared);
            Real resultLength = Mathf.Lerp(startLength, Mathf.Sqrt(endLengthSquared), weight);
            Real angle = AngleTo(to);
        ...
        /// <returns>The resulting vector.</returns>
        public static Vector2 FromAngle(Real angle)
        {
            (Real sin, Real cos) = Mathf.SinCos(angle);
        ...
        /// by the given <see cref="Real"/>.
        /// </summary>
        /// <param name="vec">The vector to multiply.</param>
        /// <param name="scale">The scale to multiply by.</param>
        /// <returns>The multiplied vector.</returns>
        public static Vector2 operator *(Vector2 vec, Real scale)
        {
            vec.X *= scale;
            vec.Y *= scale;
            return vec;
        ...
        /// <param name="vec">The vector to multiply.</param>
        /// <returns>The multiplied vector.</returns>
        public static Vector2 operator *(Real scale, Vector2 vec)
        {
            vec.X *= scale;
            vec.Y *= scale;
            return vec;
        ...
        public static Vector2 operator /(Vector2 vec, Real divisor)
        {
            vec.X /= divisor;
            vec.Y /= divisor;
            return vec;
        ...
        /// with the components of the given <see cref="Real"/>.
        /// This operation uses truncated division, which is often not desired
        /// as it does not work well with negative numbers.
        /// Consider using <see cref="PosMod(Real)"/> instead
        ...
        /// by running <see cref="Mathf.IsEqualApprox(Real, Real)"/> on each component.
        /// </summary>
        ...
        /// by running <see cref="Mathf.IsZeroApprox(Real)"/> on each component.
