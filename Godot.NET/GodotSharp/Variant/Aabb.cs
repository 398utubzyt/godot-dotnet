...
namespace Godot
{
    ...
    public struct Aabb : IEquatable<Aabb>
    {
        ...
        public readonly Real GetLongestAxisSize()
        {
            Real maxSize = _size.X;
        ...
        public readonly Real GetShortestAxisSize()
        {
            Real maxSize = _size.X;
        ...
        public readonly Aabb Grow(Real by)
        {
            Aabb res = this;
        ...
        public readonly bool IntersectsSegment(Vector3 from, Vector3 to)
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                Real segFrom = from[i];
                Real segTo = to[i];
                Real boxBegin = _position[i];
                Real boxEnd = boxBegin + _size[i];
                Real cmin, cmax;
        ...
        public Aabb(Vector3 position, Real width, Real height, Real depth)
        {
            _position = position;
        ...
        /// <param name="size">The size, typically positive.</param>
        public Aabb(Real x, Real y, Real z, Vector3 size)
        {
        ...
        /// <param name="height">The height, typically positive.</param>
        /// <param name="depth">The depth, typically positive.</param>
        public Aabb(Real x, Real y, Real z, Real width, Real height, Real depth)
        {
            _position = new Vector3(x, y, z);
            _size = new Vector3(width, height, depth);
        }
