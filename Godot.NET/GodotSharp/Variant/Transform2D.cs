...
namespace Godot
{
    ...
    public struct Transform2D : IEquatable<Transform2D>
    {
        ...
        /// </summary>
        public readonly Real Rotation => Mathf.Atan2(X.Y, X.X);
        ...
        public readonly Vector2 Scale
        {
            get
            {
                Real detSign = Mathf.Sign(BasisDeterminant());
        ...
        private readonly Real BasisDeterminant()
        {
        ...
        public readonly Transform2D InterpolateWith(Transform2D transform, Real weight)
        {
        ...
        public readonly Transform2D Rotated(Real angle)
        {
        ...
        /// <param name="angle">The angle to rotate, in radians.</param>
        /// <returns>The rotated transformation matrix.</returns>
        public readonly Transform2D RotatedLocal(Real angle)
        {
        ...
        private readonly Real Tdotx(Vector2 with)
        {
            return (this[0, 0] * with[0]) + (this[1, 0] * with[1]);
        }

        private readonly Real Tdoty(Vector2 with)
        {
        ...
        public Transform2D(Real xx, Real xy, Real yx, Real yy, Real ox, Real oy)
        {
            X = new Vector2(xx, xy);
            Y = new Vector2(yx, yy);
        ...
        public Transform2D(Real rotation, Vector2 origin)
        {
            (Real sin, Real cos) = Mathf.SinCos(rotation);
            X.X = Y.Y = cos;
            X.Y = Y.X = sin;
            Y.X *= -1;
            Origin = origin;
        ...
        /// <param name="origin">The origin vector, or column index 2.</param>
        public Transform2D(Real rotation, Vector2 scale, Real skew, Vector2 origin)
        {
            (Real rotationSin, Real rotationCos) = Mathf.SinCos(rotation);
            (Real rotationSkewSin, Real rotationSkewCos) = Mathf.SinCos(rotation + skew);
        ...
        public static Transform2D operator *(Transform2D left, Transform2D right)
        {
            ...
            Real x0 = left.Tdotx(right.X);
            Real x1 = left.Tdoty(right.X);
            Real y0 = left.Tdotx(right.Y);
            Real y1 = left.Tdoty(right.Y);
