...
namespace Godot
{
    ...
    public struct Projection : IEquatable<Projection>
    {
        ...
        /// </exception>
        public Real this[int column, int row]
        {
            readonly get
            {
                switch (column)
        ...
        public static Projection CreateDepthCorrection(bool flipY)
        {
            return new Projection(
                ...
                new Vector4(0, 0, (Real)0.5, 0),
                new Vector4(0, 0, (Real)0.5, 1)
            );
        }
        ...
        /// <returns>The created projection.</returns>
        public static Projection CreateForHmd(int eye, Real aspect, Real intraocularDist, Real displayWidth, Real displayToLens, Real oversample, Real zNear, Real zFar)
        {
            Real f1 = (intraocularDist * (Real)0.5) / displayToLens;
            Real f2 = ((displayWidth - intraocularDist) * (Real)0.5) / displayToLens;
            Real f3 = (displayWidth / (Real)4.0) / displayToLens;

            Real add = ((f1 + f2) * (oversample - (Real)1.0)) / (Real)2.0;
            f1 += add;
            f2 += add;
            f3 *= oversample;
        ...
        /// <returns>The created projection.</returns>
        public static Projection CreateFrustum(Real left, Real right, Real bottom, Real top, Real near, Real far)
        {
            if (right <= left)
            {
            ...
            Real x = 2 * near / (right - left);
            Real y = 2 * near / (top - bottom);
        ...
        /// <returns>The created projection.</returns>
        public static Projection CreateFrustumAspect(Real size, Real aspect, Vector2 offset, Real near, Real far, bool flipFov)
        {
            if (!flipFov)
            {
                size *= aspect;
        ...
        /// <param name="zFar">The far clipping distance.</param>
        /// <returns>The created projection.</returns>
        public static Projection CreateOrthogonal(Real left, Real right, Real bottom, Real top, Real zNear, Real zFar)
        {
            Projection proj = Projection.Identity;
            proj.X.X = (Real)2.0 / (right - left);
            proj.W.X = -((right + left) / (right - left));
            proj.Y.Y = (Real)2.0 / (top - bottom);
            proj.W.Y = -((top + bottom) / (top - bottom));
            proj.Z.Z = (Real)(-2.0) / (zFar - zNear);
        ...
        /// <param name="flipFov">If the field of view is flipped over the projection's diagonal.</param>
        /// <returns>The created projection.</returns>
        public static Projection CreateOrthogonalAspect(Real size, Real aspect, Real zNear, Real zFar, bool flipFov)
        {
            if (!flipFov)
            {
                size *= aspect;
        ...
        /// <param name="flipFov">If the field of view is flipped over the projection's diagonal.</param>
        /// <returns>The created projection.</returns>
        public static Projection CreatePerspective(Real fovyDegrees, Real aspect, Real zNear, Real zFar, bool flipFov)
        {
            if (flipFov)
            {
                fovyDegrees = GetFovy(fovyDegrees, (Real)1.0 / aspect);
            }
            Real radians = Mathf.DegToRad(fovyDegrees / (Real)2.0);
            Real deltaZ = zFar - zNear;
            (Real sin, Real cos) = Mathf.SinCos(radians);
        ...
        /// <returns>The created projection.</returns>
        public static Projection CreatePerspectiveHmd(Real fovyDegrees, Real aspect, Real zNear, Real zFar, bool flipFov, int eye, Real intraocularDist, Real convergenceDist)
        {
            if (flipFov)
            {
                fovyDegrees = GetFovy(fovyDegrees, (Real)1.0 / aspect);
            ...
            Real ymax = zNear * Mathf.Tan(Mathf.DegToRad(fovyDegrees / (Real)2.0));
            Real xmax = ymax * aspect;
            Real frustumshift = (intraocularDist / (Real)2.0) * zNear / convergenceDist;
            Real left;
            Real right;
            Real modeltranslation;
            switch (eye)
            {
                case 1:
                    left = -xmax + frustumshift;
                    right = xmax + frustumshift;
                    modeltranslation = intraocularDist / (Real)2.0;
                    break;
                case 2:
                    left = -xmax - frustumshift;
                    right = xmax - frustumshift;
        ...
        public readonly Real GetAspect()
        {
        ...
        public readonly Real GetFov()
        {
            Plane rightPlane = new Plane(X.W - X.X, Y.W - Y.X, Z.W - Z.X, -W.W + W.X).Normalized();
            if (Z.X == 0 && Z.Y == 0)
            {
                return Mathf.RadToDeg(Mathf.Acos(Mathf.Abs(rightPlane.Normal.X))) * (Real)2.0;
            }
            else
            {
        ...
        public static Real GetFovy(Real fovx, Real aspect)
        {
            return Mathf.RadToDeg(Mathf.Atan(aspect * Mathf.Tan(Mathf.DegToRad(fovx) * (Real)0.5)) * (Real)2.0);
        }
        ...
        public readonly Real GetLodMultiplier()
        {
            if (IsOrthogonal())
            {
            ...
            {
                Real zn = GetZNear();
                Real width = GetViewportHalfExtents().X * (Real)2.0;
                return (Real)1.0 / (zn / width);
            }
        }
        ...
        public readonly int GetPixelsPerMeter(int forPixelWidth)
        {
            ...
            return (int)((result.X * (Real)0.5 + (Real)0.5) * forPixelWidth);
        }
        ...
        public readonly Projection PerspectiveZNearAdjusted(Real newZNear)
        {
            Projection proj = this;
            Real zFar = GetZFar();
            Real zNear = newZNear;
        ...
        public readonly Projection Inverse()
        {
            ...
            Real pvt_val; /* Value of current pivot element */
            Real hold; /* Temporary storage */
            Real determinant = 1.0f;
            for (k = 0; k < 4; k++)
            {
