...
namespace Godot
{
    ...
    public struct Quaternion : IEquatable<Quaternion>
    {
        ...
        public readonly Real AngleTo(Quaternion to)
        {
            Real dot = Dot(to);
        ...
        public readonly Quaternion SphericalCubicInterpolate(Quaternion b, Quaternion preA, Quaternion postB, Real weight)
        {
#if DEBUG
            if (!IsNormalized())
            {
        ...
        /// <returns>The interpolated quaternion.</returns>
        public readonly Quaternion SphericalCubicInterpolateInTime(Quaternion b, Quaternion preA, Quaternion postB, Real weight, Real bT, Real preAT, Real postBT)
        {
#if DEBUG
            if (!IsNormalized())
            {
        ...
        public readonly Real Dot(Quaternion b)
        {
        ...
        public readonly Quaternion Slerp(Quaternion to, Real weight)
        {
#if DEBUG
            if (!IsNormalized())
            {
            ...
            Real sinom, scale0, scale1;

            // Calculate coefficients.
            if (1.0 - cosom > Mathf.Epsilon)
            {
                // Standard case (Slerp).
                Real omega = Mathf.Acos(cosom);
        ...
        /// <param name="weight">A value on the range of 0.0 to 1.0, representing the amount of interpolation.</param>
        /// <returns>The resulting quaternion of the interpolation.</returns>
        public readonly Quaternion Slerpni(Quaternion to, Real weight)
        {
#if DEBUG
            if (!IsNormalized())
            {
            ...
            Real theta = Mathf.Acos(dot);
            Real sinT = 1.0f / Mathf.Sin(theta);
            Real newFactor = Mathf.Sin(weight * theta) * sinT;
            Real invFactor = Mathf.Sin((1.0f - weight) * theta) * sinT;
        ...
        public Quaternion(Vector3 axis, Real angle)
        {
#if DEBUG
            ...
            {
                (Real sin, Real cos) = Mathf.SinCos(angle * 0.5f);
                Real s = sin / d;
        ...
        {
            Real halfA1 = eulerYXZ.Y * 0.5f;
            Real halfA2 = eulerYXZ.X * 0.5f;
            Real halfA3 = eulerYXZ.Z * 0.5f;

            // R = Y(a1).X(a2).Z(a3) convention for Euler angles.
            // Conversion to quaternion as listed in https://ntrs.nasa.gov/archive/nasa/casi.ntrs.nasa.gov/19770024290.pdf (page A-6)
            // a3 is the angle of the first rotation, following the notation in this reference.

            (Real sinA1, Real cosA1) = Mathf.SinCos(halfA1);
            (Real sinA2, Real cosA2) = Mathf.SinCos(halfA2);
            (Real sinA3, Real cosA3) = Mathf.SinCos(halfA3);
        ...
        /// by the given <see cref="Real"/>. This operation is not
        /// meaningful on its own, but it can be used as a part of a
        /// larger expression.
        /// </summary>
