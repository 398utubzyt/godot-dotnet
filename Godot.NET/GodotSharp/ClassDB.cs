...
namespace Godot
{
    public partial class ClassDB
    {
        ...
        private static unsafe void CreateClassDBClass(void* data, nint instance, nint* args, long count, nint ret, CallError* error)
        {
        ...
        private static class ManagedHelper
        {
            ...
            {
                Instance<RefCounted>(managed)?.Reference();
            }
            [UnmanagedCallersOnly]
            public static unsafe void Unreference(nint managed)
            {
                Instance<RefCounted>(managed)?.Unreference();
            }
            [UnmanagedCallersOnly]
            public static unsafe ulong GetRid(nint managed)
            {
                return Instance<Resource>(managed)?.GetRid().Id ?? 0;
            }
            [UnmanagedCallersOnly]
            ...
            {
                Console.WriteLine($"Virtual get: {new StringName((nint)type)}");
            ...
            where T : GodotObject
        {
            ...
            Console.WriteLine($"Register Class: {cn} (from {pcn})");
